defmodule TrialGroup.AdministrationTest do
  use TrialGroup.DataCase

  alias TrialGroup.Administration

  import TrialGroup.AdministrationFixtures
  alias TrialGroup.Administration.{Admin, AdminToken}
  ...
  describe "apply_admin_email/3" do
    ...
    test "validates email uniqueness", %{admin: admin} do
      ...
      {:error, changeset} = Administration.apply_admin_email(admin, password, %{email: email})
    ...
    test "sends token through notification", %{admin: admin} do
      token =
        extract_admin_token(fn url ->
          Administration.deliver_admin_update_email_instructions(admin, "current@example.com", url)
        end)
  ...
  describe "update_admin_email/2" do
    setup do
      ...
      token =
        extract_admin_token(fn url ->
          Administration.deliver_admin_update_email_instructions(%{admin | email: email}, admin.email, url)
        end)
  ...
  describe "delete_admin_session_token/1" do
    test "deletes the token" do
      admin = admin_fixture()
      token = Administration.generate_admin_session_token(admin)
      assert Administration.delete_admin_session_token(token) == :ok
      refute Administration.get_admin_by_session_token(token)
    end
  end
